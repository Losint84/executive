...
let SecondaryTag = settings.SecondaryTag

module.exports = {
conf: {name: 'kadın', aliases: ["kız", "k"], help: "!kadın @Etiket/ID İsim Yaş"},
...
await member.roles.add(role.WomanRoles);
member.roles.remove(role.UnregisterRole);

Register.find({UserID: member.id}, async(err, data) => {
if(err) console.log(err)
if(data.length < 1) return message.channel.send(embed.setDescription(`${member} üyesine ${message.author} tarafından ${role.WomanRoles.map(x => `<@&${x}>`)} rolleri verildi.`).setColor('BLACK'))

if(data.length > 0) {   
let names = data.reverse().reverse()

message.channel.send(embed.setDescription(`
${member} üyesine ${message.author} tarafından ${role.WomanRoles.map(x => `<@&${x}>`)} rolleri verildi.
...
Kişinin önceki isimlerine \`!isimler @üye\` komutuyla görüntülüyebilirsiniz.`).setColor('BLACK'))}
...
staffData.WomanTotal++
staffData.save();

let userData = new Register({GuildID: message.guild.id, UserID: member.id, Admin: message.author.id, UserNames: GuildName, Role: `<@&${role.WomanRole}>`, Sex: "Erkek", Date: Date.now()})
userData.save().catch(err => console.log(`Kadın datası kayıt edilirken bir sorun ile karşılaştım.`))
}) 
